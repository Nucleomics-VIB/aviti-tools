...
def test_resolve_tile_spec_default_single_lane(tmp_path):
    ...
    # No anchors — the worker pairs this with --exclude-tile L.R..C..S.
    # via pipeline_invocation, which is what actually restricts the run.
...
def test_resolve_tile_spec_all(tmp_path):
    ...
    # "all" means no restriction — both --include-tile and --exclude-tile
    # are omitted so bases2fastq processes everything it would by default.
    assert out["pattern"] is None
    assert out["spec"] == "all"
    # Full tile inventory still travels with the row for UI display.
