...
if (nrow(valid_results_df) > 0) {
  ...
  image_summary <- valid_results_df %>%
    select(image, colored_percentage, color_balance_outcome, signal_bg_outcome, integrated_score)
...
cat("Batch processing complete. Results in", outdir, "\n")
cat("Summary report:", summary_report, "\n")

if (params$cores > 1) {
  cat("Check parallel processing progress in:", file.path(outdir, "parallel_progress.txt"), "\n")
}